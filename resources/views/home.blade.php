        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                @foreach ($rooms as $room)
                    @php
                        $primaryPhoto = $room->roomPhotos->first();
                        $photoUrl = $primaryPhoto ? $primaryPhoto->photoUrl : 'https://placehold.co/600x400/312e81/ffffff?text=No+Photo';
                    @endphp
                    
                    <div class="bg-white rounded-sm overflow-hidden card-shadow hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 border border-gray-200">
                        
                        <div class="h-48 bg-indigo-100 relative">
                            <img 
                                src="{{ $photoUrl }}" 
                                alt="Photo of Room {{ $room->roomNumber }}" 
                                class="w-full h-full object-cover"
                                onerror="this.onerror=null;this.src='https://placehold.co/600x400/orange/white?text=placeholder'"
                            >
                            <span class="absolute top-3 right-3 bg-white text-brand-600 text-sm font-extrabold px-3 py-1 rounded-sm shadow-lg">
                        ...
                        <div class="p-4 pb-2">
                            ...
                            <div class="flex items-center space-x-2 mb-4 text-gray-700">
                                <span class="flex items-center gap-1 text-sm font-medium @if($room->tv) text-green-600 @else text-red-500 @endif">
                                    @if($room->tv)
                                ...
                                <span class="flex items-center gap-1 text-sm font-medium @if($room->wifi) text-green-600 @else text-red-500 @endif">
                                    @if($room->wifi)
                        ...
                        <div class="p-6 pt-0">
                            <button class="cursor-pointer w-full bg-brand-500 text-white font-semibold py-2 rounded-sm hover:bg-brand-600 transition duration-150">
                                View Details & Book
                            </button>
                        </div>
                    </div>
